...
class InceptionResnetV1(nn.Module):
    ...
    """
    def __init__(self, fine_turn = 0, pretrained=None, classify=False, num_classes=10575, dropout_prob=0.6, device=None):
        super().__init__()
        ...
        self.centers = torch.zeros(num_classes, 512).type(torch.FloatTensor)#特征向量512
        self.pred = 0
    ...
    def forward(self, x):
        ...
        self.features_norm = x
        self.pred = F.log_softmax(self.logits(x))
        if self.classify:
            x = self.logits(x)
        return x
    def get_center_loss(self, target, alpha):
        ...
        center_loss = criterion(self.features,  centers_batch)#中心特征
